...
#время работы решения, определяемого chromosome для задачи jobs
...
#Создание популяции. Популяция - массив хромосом, хромосома - массив субхромосом, субхромосома - массив пар (номер работы и номер операции в работе)
...
def LOX(parent_1, parent_2, border_1, border_2): 
...
#оператор crossover, дважды применяющие LOX для получения двух "детей" от двух "родителей"
...
#перестановка местами элементов в одной случайно выбранной хромосоме
...
#оператор, совмещающий crossover и mutation, применяющийся к хромосомам в рамках алгоритма
...
#сортировка популяции по возрастанию значения фитнеса
def sort_population(population, fitnesses):
    ...
    for i in range(len(population)):
        ...
        weight.append(max_fit - fitness_and_population[i][0]) 
 ...
 #m - число машин, jobs - массив работ, работа - массив операций, операция - пара чисел (номер машины и время выполнения)
def main(m, jobs, generations_amount, population_size, optimum):
    ...
    for _ in range(generations_amount):
        ...
        for i in range(5):
            a, b = random.choices(population, weights = weight, k = 2) #выбор детей пропорционально тому насколко хороши показания фитнеса
        ...
        if min_fit != max_fit:
            for i in range(population_size):
                weight[i] = max_fit - fitnesses[i]

    return min_fit

#получение конкретной задачи из бенчмарка
def take_instance(instance_name):
    ...
    with open('JSPLIB-master/' + instance['path']) as f:
        data = list(f)[5:]
...
for i in range(40):
    max_res = 1651 #значение полученное генетическим алгоритмом для максимизации
    length = max_res - optimum
    res = main(m, jobs, 500, 50, optimum)
    res -= optimum
    res /= length
    res *= 100
    res_arr.append(res)
...
print("дисперсия: ", np.var(res_arr))
print("медиана: ", np.median(res_arr))
print("25 процентиль: ", np.percentile(res_arr, 25))
print("50 процентиль: ", np.percentile(res_arr, 50))
print("75 процентиль: ", np.percentile(res_arr, 75))