...
#Вариант реализации алгоритма, для выведения графика для конкретной задачи
...
def main(m, jobs, generations_amount, population_size, optimum): #m - число машин, jobs - массив работ, работа - массив операций, операция - пара чисел (номер машины и время выполнения)
    ...
    res *= 100
    ax_y.append(res)
    res_on_iter[0].append(res)


    for _ in range(generations_amount-1):
        children = []
...
m, jobs, optimum = take_instance('ft06')    
res_on_iter = [[] for i in range(500)]
max_res = 87

for i in range(10):
    length = max_res - optimum
    ax_res = main(m, jobs, 500, 50, optimum)
    plt.plot(ax_x, ax_res)

for i in range(500):
    ax_disp.append(np.var(res_on_iter[i]))
    ax_med.append(np.median(res_on_iter[i]))
    ax_per_25.append(np.percentile(res_on_iter[i], 25))
    ax_per_50.append(np.percentile(res_on_iter[i], 50))
    ax_per_75.append(np.percentile(res_on_iter[i], 75))



plt.plot(ax_x, ax_disp, label='дисперсия')
plt.plot(ax_x, ax_med, label='медиана') 
plt.plot(ax_x, ax_per_25, label='25 процентиль')
plt.plot(ax_x, ax_per_50, label='50 процентиль')
plt.plot(ax_x, ax_per_75, label='75 процентиль')
plt.legend()
plt.show()